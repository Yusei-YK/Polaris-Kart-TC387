...
/* 电机极性 KART_STEER/LEFT/RIGHT_MOTOR_SIGN → kart_calib.h 第二节 */
...
/* 编码器极性 KART_LEFT/RIGHT_ENCODER_SIGN → kart_calib.h 第二节
 * 脉冲当量 KART_LEFT/RIGHT_ENC_PULSE_TO_M → kart_calib.h 第三节 */
 ...
 *   与 TC387 的 ASCLIN 电平直接兼容，不需电平转换。
 ...
 *     语音模块也在这个口，真冲突 → 选它时必须同时 KART_LOG_ON_UART0=1
 *     （日志改走 USB-TTL 直插），否则下面的 #error 会拦住。 */
/* 总开关。为何它在 board_pins.h 而不在 kart_person_link.h：
 * 下面 KART_DOT_MATRIX_MUTED 的推导要用到它，而点阵屏/灯板驱动（Kart_TPL）
 * 只 include board_pins.h，不能反过来 include Kart_App 的头。
 * 放在这里也符合分层：“哪个外设归谁”本来就是 Kart_Config 的职责。
 *
 * 0 = 完全不出代码、不动 UART 归属，工程行为与没这个模块一模一样。
 * 1 = 接管 KART_PERSON_LINK_PORT 选定的 UART；选 LIGHT 时点阵屏自动静默。
 * 默认 0：硬件没接 4D7 时开着毫无意义，而且会白白丢掉点阵屏与菜单回显。 */
/* 【视觉源一键切换】只改下面这一个值：
 *   0 = TC387 本地 SCC8660 视觉（KART_S3_FOLLOW_SRC_VISION）
 *   1 = TC4D7 / PLINK 视觉（KART_S3_FOLLOW_SRC_PLINK）
 * kart_mission.h 会自动联动控制源，不需要再改第二个宏。 */
#define KART_PERSON_LINK_ENABLE         (0)

#define KART_PERSON_LINK_PORT_LIGHT     (0)
#define KART_PERSON_LINK_PORT_VOFA      (1)

#ifndef KART_PERSON_LINK_PORT
#define KART_PERSON_LINK_PORT           (KART_PERSON_LINK_PORT_LIGHT)
#endif

#if (KART_PERSON_LINK_PORT == KART_PERSON_LINK_PORT_LIGHT)
...
#define KART_DOT_MATRIX_MUTED           (KART_PERSON_LINK_ENABLE && (KART_PERSON_LINK_PORT == KART_PERSON_LINK_PORT_LIGHT))
...
#define KART_VOICE_MUTED                (KART_PERSON_LINK_ENABLE && (KART_PERSON_LINK_PORT == KART_PERSON_LINK_PORT_VOFA))

#if (KART_PERSON_LINK_ENABLE && (KART_PERSON_LINK_PORT == KART_PERSON_LINK_PORT_VOFA) && !KART_LOG_ON_UART0)
#error "person_link 选了 VOFA 口(UART_10) 但日志也在 UART_10：把 KART_LOG_ON_UART0 改 1，或把 PORT 改回 LIGHT"
#endif
 ...
 *   正是【转向绝对值编码器】(见上面 KART_STEER_ABS_*)。
 ...
 *   KART_DOT_ROWS_TEST(=0) 卡着，正常开机流程从来没执行过。
 ...
 *   仅 KART_WIFI_ENABLE=1 时生效（=0 时保留 SYNC 边沿诊断计数）。
 *   P33.5 除无线模块 RST 外无其他用途。
 *
 * KART_WIFI_ENABLE 只控制图传本身；不再联动关闭 IPS200 菜单。 */
#ifndef KART_WIFI_ENABLE
/* 2026-08-12 停用 WIFI6B21-SPI；图传改走下载器 UART_0(P14.0/P14.1)。 */
#define KART_WIFI_ENABLE                (0)
#endif
 ...
 *          就绪/科目三界面不必再屏蔽菜单 MID,menu 与 mission 也不会再对同一脚
