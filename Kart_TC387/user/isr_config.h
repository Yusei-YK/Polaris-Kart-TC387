...
/* 2026-07-26 由 52 降到 9:CCU61_CH0 现为点阵屏 1ms 软扫时基,ISR 内调
 * tld7002_set_duty() 发 23 字节 @2M 半双工,回环字节要靠 uart1_rx_isr(prio 14)搬走,
 * 而 uart1_rx_buffer 只有 1 字节深(见 zf_driver_uart.c)。若本中断优先级高于 14,
 * ISR 全程压着 RX 不放 → 回环字节被覆盖丢弃 → TLD7002 回读恒 COMM_ERROR。
 * 取 9(低于 UART0_RX=10 与 UART1_RX=14)让串口中断能抢进来。 */
#define CCU6_1_CH0_ISR_PRIORITY 9
