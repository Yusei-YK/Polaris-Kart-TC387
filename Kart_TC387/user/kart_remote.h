 ...
 * (速度环是闭环,目标只有 20,换满电电池也不会更快)。07-27 恢复 60。
 * 【必须与 kart_playback.h 的 KART_PLAYBACK_SPEED_MAX 同步】否则录得快、复现被钳慢。 */
#define KART_REMOTE_MAX_SPEED       (50.0f)
#define KART_REMOTE_STEER_SIGN      (+1)     /* 方向映射符号:上车若打反改 -1(最终仍硬钳软限位) */
