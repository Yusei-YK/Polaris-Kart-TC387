...
typedef enum
{
    ...
    S4_PHASE2_REVERSE,      /* 第二阶段:开环倒车原路返回(判停自动触发,不用按键) */
    S4_FINISHED,            /* 返回完成 */
    S4_FAULT,
 ...
 * START 键仍保留为手动提前触发,不想等这 500ms 时可以按。 */
#define KART_S4_AUTOSTOP_ARM_DIST   (1.0f)      /* 解锁自动判停的最小行驶里程(m) */
#define KART_S4_AUTOSTOP_SPEED_EPS  (2.0f)      /* 停住判据(脉冲/5ms) */
#define KART_S4_AUTOSTOP_HOLD_TICKS (50U)       /* 连续停住拍数(10ms/拍 → 500ms) */
