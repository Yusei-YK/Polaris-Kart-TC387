 ...
 * dot_scan 已经没有调用点了:code/ 和 user/ 里 kart_multicore_dot_scan 只剩
 * 下面这行声明和 kart_multicore.c 里的定义。点阵扫描早已交给 CPU0 的 1ms PIT
 * (cc61_pit_ch0_isr),不再经过这个包装。另外四个还在用:imu/odom 在 isr.c 的
 * 5ms 中断里,record_poll 在 10ms 拍,dot_show_string 在 100ms 拍。留着
 * dot_scan 是因为 COMPAT 通道整套都留着。 */
