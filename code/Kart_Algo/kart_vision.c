 ...
 * 2026-08-13 改动:先建掩膜再做十字腐蚀,和逐飞官方那套对齐。
 *            【注意腐蚀只在第三趟跑】kart_vision_erode_once() 全文件只有一个
 *            调用点,在 kart_vision_process 的面积门之后,只作用于已经隔离
 *            出来的团块,用来算稳定中心。
 *            找种子和生长期间掩膜是没腐蚀过的,所以 run 长度门
 *            VISION_MIN_RUN(3) 仍然在干活,不是"退化成保险"。
...
/*-------------------------------------------------------------------------------------------------------------------
 * 建掩膜:每像素存色相偏差 dev 本身(0~126,127=哨兵,详见文件顶部 VIS_DEV 那段)。
 * 【不是三态标签】老实现存 0/1/2(背景/宽松候选/高置信种子),08-16 改成存 dev,
 * 阈值判断推迟到用的时候,回退才能拿不同的门重跑同一张图。
 *-----------------------------------------------------------------------------------------------------------------*/
 ...
 * 这是逐飞那套流程里的「腐蚀迭代」:一遍腐蚀就能杀掉任何一个方向上窄于 3px 的
 * 结构,包括纵向的毛刺和一像素宽的斜反光线,而 run 长度门对纵向的东西无能为力。
 * 【但两者不是替代关系】本函数只在 kart_vision_process 的第三趟被调一次,
 * 那时团块已经选定,腐蚀结果只用于算中心;找种子/生长阶段用的是没腐蚀过的掩膜,
 * 那一段仍然靠 VISION_MIN_RUN(3) 挡短段。
 *
 ...
 *
 * 【MIN_RUN 仍然在干活】不要按"腐蚀已经收拾过噪点"来理解:
 * kart_vision_erode_once() 全文件只在 kart_vision_process 的第三趟调用,
 * 在面积门之后,那时种子和生长都已经做完了。所以在找种子/生长这条路上,
 * 长度 1-2 的横向段
 * 完全靠这里的 VISION_MIN_RUN(3) 挡掉 —— 改成 1 会让单像素
 * 噪点也有资格起团。ERODE_ITER 当前是 1,改成 0 只影响中心精度。
 *-----------------------------------------------------------------------------------------------------------------*/
 ...
 * 估算种子 run 中点处的纵向支撑长度
 *
 * 从 (cx, y0) 沿列向上、向下走,记住最远那个 dev < tol 的行,返回 bottom-top+1。
 * 中途允许断:连续断超过 VISION_MAX_SKIP(3) 行才停,
 * 所以板子上被身体遮出的横向缺口不会把跨度截断。
 *
 * 【为什么需要它】只按 run 长度选种子会挑中地面反光那种又长又薄的一条。
 * vision_try_detect 用 min(run 长度, 本函数返回的跨度) 打分,同分再比两者之积,
 * 这样厚的二维物体优先于细长条。读的是没腐蚀过的掩膜。
 *-----------------------------------------------------------------------------------------------------------------*/
 ...
 *
 * 四趟：
 *   第零趟 建色相偏差图(每像素求一次色相),存 dev 本身而不是三态标签,不做前置腐蚀。
 *   第一趟 全图逐行取候选 run,只有含 dev < SEED_TOL 像素的 run 有资格起团;
 *          在这些 run 里按 min(run 长度, 纵向支撑跨度) 取最高分,同分再比两者之积 ——
 *          【不是取最长的那条】取最长会挑中地面反光那种又长又薄的一条。
...
const kart_vision_result_t *kart_vision_process(const uint16 *img, int16 w, int16 h)
{
    ...
    /* 太窄:框太窄时宽高比和连续性尺度门都失去意义,量化误差比测量值本身还大。
     * 【测距不看宽度】dist_m 是由框高 bh 反算的(本函数末尾那行
     * res->dist_m = BOARD_H_M * FPX / bh * lens_range_scale),宽度只喂两处:
     * 这道门下面的宽高比门(bw*10/bh),和连续性门里的"一帧内宽度翻倍/减半"判断。 */
 ...
 *
 * side 上限 31 → 最多 961 个样本;插入排序 O(n^2),
 * 单条序列最坏 46 万次比较上下,而下面一次排 3 条(色相/饱和度/亮度和),
 * 合计约 138 万次;
 ...
 * 目标黄的中心是 VISION_HUE_CENTER(44)、品红约 210,都远离 0/251 边界,不受影响。
 * (原文把黄的中心写成了 42,那是 VISION_HUE_SECTOR 的值;
 *  真正的中心 08-16 标到了 44,数已订正,结论不变。) */
    for(i = 1; i < n; i++)
    {
        int16 kh = hbuf[i];
        int16 ks = sbuf[i];
 ...
 * 调试页那句 "M%6ld >%6ld" 不用改,按现在的 VISION_HUE_TOL(16)显示出来
 * 就是"容差 16 > 偏差 3"(原文举的 6 是 TOL 还等于 6 时写的)。
 *
