...
void kart_pedal_poll(uint16 period_ms)
{
    ...
    if(MISSION_PEDAL == kart_mission_get_mode())
    {
         ...
         * 【为什么这里还要自己滤】挡位不能只看一帧:链路掉的时候 kart_remote 会把
         * 挡位钳成低挡,而 remote_online 来一帧就当拍回 1,于是每次链路恢复都有
         * 几拍"在线 + 钳出来的低挡"。不加门的话链路每抖一次就误急停一次,跟开关
         * 实际在哪儿无关 —— 现场表现就是屏幕反复闪回主菜单。所以这里两道门:
         * 先等链路坐稳,再要求已武装。 */
        if(0U == kart_remote_is_online())
        {
            pedal_rc_online_run = 0;
...
void kart_pedal_control_update(void)
{
    ...
    else if(0U != pedal_brake)
    {
        ...
        else
        {
             ...
             * 【为什么要清积分】只做前两件事的话刹车是软的:巡航的稳态 duty 全在
             * 积分器里(4.0m/s 约 8913),它要按每拍 54.3 退掉近 1 秒,这期间它一直在
             * 抵消 P 项给出的反向 duty。完整推导和数字写在 kart_control.h 的
             * kart_control_brake_hard() 声明处。
