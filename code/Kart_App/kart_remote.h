 ...
 *   接管层已实现(kart_remote_control_update(),由 kart_mission 三处调用)。
 *   下面三条是接管必须一直满足的不变量,改这个模块时不许破:
 *     ① SBUS→转角目标钳到软限位 KART_STEER_DELTA_LIMIT_L/R;
 *        映射后还有一道硬钳,见 kart_remote.c:260。
 *     ② 失联超时兜底(转向回中 + 速度清零 + 关使能);
 *        失联时先断转向电机再急停,且一个通道都不读,见 :320。
 *     ③ 帧校验,坏帧丢弃不更新目标,见 :97。
 * ------------------------------------------------------------------
 */
...
/* -------------------- 失联判据 -------------------- */
/* 10ms 拍调 kart_remote_poll() 检查:若自上次有效帧以来
 ...
 * 当前值 50。曾经下调到 20,那一档科目一录制与复现速度只剩 1/3
 * (速度环是闭环,目标只有 20,换满电电池也不会更快),别再往回调。
 * 本值只决定【录制/遥控手感】,不决定复现快慢:剖面(PB Prof)出厂打开后,
 * 复现速度由路径几何算,与录制速度无关,不需要跟 KART_PLAYBACK_SPEED_MAX
 * 同步。录制反而应该录慢些:路径更准、不打滑。 */
 ...
 * period_ms = 本次调用间隔。当前由 10ms 拍调用,
 * cpu0_main.c 传 2U * KART_MAIN_LOOP_PERIOD_MS。 */
