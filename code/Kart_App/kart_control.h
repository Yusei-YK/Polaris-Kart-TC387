 ...
 *
 * 取 0.4 的依据是物理量,不是"听着够缓":
 *   0.4 脉冲/5ms 每拍 ÷ 0.005s = 80 (脉冲/5ms)/s × 0.0736 = 5.9 m/s²,
 * 略高于抓地上限,所以既不人为拖慢加速(先打滑的是轮胎,斜坡不是瓶颈),
 * 又能真正把 PID 误差压在小量。0→60 脉冲 = 150 拍 = 750ms,与"22kg / 300W
 * 加到 4.4 m/s"的物理时间同量级。
 * 【别往大调】1.5 换算过来是 22 m/s²,而整车实际能给的只有 2×150W ÷ 22kg 在
 * 4 m/s 处 P/(m·v) ≈ 3.4 m/s²、EVA 硬轮抓地上限 0.5~0.7g ≈ 5~7 m/s²。斜坡
 * 比车跑得出来的快 5~9 倍时,目标速度一瞬间就跑到实际速度前面去,误差照样
 * 一次性张开,上面那条病因链(①~④)一条都没被治住,只是被"看起来有斜坡"
 * 掩盖了。起步窜车的真因在这里,不是编码器标定。 */
 ...
 * 【保持关闭】不是"来不及验",是量程本身就配不上。按代码里的标定量:
 *   满舵 R = KART_STEER_R_TIMES_DELTA / 软限位 = 1480/1064 = 1.39m
 *            (两个量都在 kart_calib.h,后者是 KART_STEER_DELTA_LIMIT_L)
