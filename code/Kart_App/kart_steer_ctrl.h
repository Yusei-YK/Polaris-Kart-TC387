 ...
 * 真凭据只有第一条:① 实车科目一/科目三走线已跑通。
 * ② 这条原来算错了:内环静摩擦死区 950/15 = 63 计数,除以 30 计数/度是 2.1°,
 *    不是 0.83°(0.83 = 25/30,分子错拿了 kart_motion.h 的 25)。
 *    即 2.1° 以内的航向误差不动方向盘 —— 比原文以为的宽 2.5 倍。
 *    1.50m 前视下 2.1° 约 5.5cm 横向偏差,绕桩还吃得住,所以 30 不动;
 *    但再提速后若出现走线滞后,这个死区是第一个该看的地方:提 KART_HEAD_KP
 *    (度→计数)或提内环 Kp(计数→duty)都能压小它,代价都是过冲变大。
