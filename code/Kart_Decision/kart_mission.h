 ...
 * 【2026-09-07 数一下】"十余处"偏保守:kart_menu.c 里非注释的调用点是 21 处
 * (踏板页、五条科目入口、遥控页、退出路径各有几处)。改菜单结构时要意识到
 * 这个数量级 —— 模式切换不是集中在一个地方做的。
 ...
 * 【2026-09-07 订正这个标题】标题写的"100 → 100U"跟下一行自己的正文对不上:
 * 正文说旧值是 10U(100ms),所以这一节讲的是 10U → 100U,不是 100 → 100U。
 * 后缀 U 那件事不是本节的主题(值本身翻了十倍才是)。正文全对,只是标题写错。
 ...
 * 【2026-09-07 补:看 CH5 之前先确认日志分档】CH5 = core3 单帧耗时只在出厂
 * 43 通道档(LOG_PROFILE_S3=1)成立;51 通道全量档的 CH5 是左轮实测速度,
 * 拿它算这个值会得出完全无关的数。与 kart_follow.h / kart_playback.h
 * 那两处是同一个坑。 */
