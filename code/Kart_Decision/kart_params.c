...
static const kart_param_meta_t param_meta[PARAM_MAX] =
{
     ...
     * 【2026-09-06 订正】94 脉冲这笔账是对的,但"上限 95"和本槽的 max 从来没对上 ——
     *   本槽是 150,PB Clamp 和 RC Vmax 也是 150。真按这条理由封在 95 的只有
     *   PB Vmin(95)和 S3 OLSpd(-95)。git log -S 查过:这句话和那三个 150 是同一次
     *   提交(dd96d93,换齿轮重标那次)一起落下的,所以不是后来漂的,一开始就不一致。
     *   后果正是这句自己说的"空的":150 脉冲 = 11.0 m/s = 39.7 km/h,顶上那 55 个
     *   计数根本跑不出来,现场把这三项拨过 95 就没有意义。量程赛后不动,只订正说法。
     * 【现场必读】几何最大曲率(满舵 R=1480/1064=1.39m → κ=0.719/m,见 kart_calib.h 的 KART_STEER_MIN_RADIUS_M;原注释写的 R=1.32m、κ=0.76/m 是重标中位之前的值,已订正)对应 sqrt(4.0/0.719)=
     * 2.36 m/s = 32 脉冲(原注释写的 2.29 m/s = 31 脉冲是上一行那个旧曲率算出来的,门限跟着抬一格)。Vmax ≤ 32 时剖面整段被压平成常数,弯道整形完全不起作用,
 ...
 *   【2026-09-06 补】上面那段是当时的现场记录,别当现值看:这个宏后来又提到了
 *   4.0f(kart_calib.h)。按 4.0 反解是 1.3*(4.0+0.10)/0.00046 = 11587,已经超过
 *   CEIL,所以 param_derive_imax() 返回的是被夹住的 10000,那 1.3 倍余量实际只
 *   拿到 1.12 倍。够用 —— 10000 duty 按同一条拟合支撑 4.5 m/s,仍高于 4.0;
 *   但要再往上提踏板上限,反解这条路已经到顶,得从 duty 满量程本身想办法。
 ...
 *   【2026-09-06 说明 27% 和 31%】本文件上方 Spd Imax 那行(和 kart_params.h)写的是
 *   27%,这里写 31%,是同一道墙的两种量法,都对:27% = 10000-7300,拿日志实测峰值
 *   duty 算;31% = 10000-6876,拿稳态 Kp*e+I 算。别拿一个去改另一个。
