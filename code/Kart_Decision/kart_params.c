...
static const kart_param_meta_t param_meta[PARAM_MAX] =
{
     ...
     * "上限 95" 只对得上 PB Vmin(95) 和 S3 OLSpd(-95):本槽、PB Clamp、RC Vmax 的
     *   max 都是 150,那 55 个计数(150 脉冲 = 11.0 m/s = 39.7 km/h)整车跑不出来,
     *   现场把这三项拨过 95 没有意义。量程赛后不动。
     * 【现场必读】几何最大曲率(满舵 R=1480/1064=1.39m → κ=0.719/m,见 kart_calib.h
     *   的 KART_STEER_MIN_RADIUS_M)对应 sqrt(4.0/0.719)= 2.36 m/s = 32 脉冲。
     *   Vmax ≤ 32 时剖面整段被压平成常数,弯道整形完全不起作用,只剩终点锚点刹车。
     *   想要真正的过弯减速必须把 Vmax 放到 40~50。 */
    ...
    /* 保留占位项，避免后续参数的 Flash 下标整体移动。运行时和菜单均不使用 —— 菜单 Settings 不按 param_id_t 下标铺行,走的是显式的 set_rows[] 表,本项不在表里,所以连显示都没有。原注释写的"保留两个"到 2026-08-22 只剩一个:下面那个 Reserved 2 已改作 S3 Lead。 */
    { "Reserved 1", -95.0f,  -1.0f,   1.0f, -25.0f,                             0 },
    /* S3 Lead 复用了原 "Reserved 2" 占位槽:量程 0.05..5.00 / 步长 0.05 /
     * 小数位 2 一个字没动(它本来就是米量纲),槽位序号和 PARAM_MAX 也没动,
     * 所以 Flash 旧存档照样读得回来。出厂值 1.00 = 实测过线 1.5m 减去可接受的
     * 0.5m,来历见 kart_playback.h。 */
    ...
    /* 横向增益(计数/米):满舵 1064 计数(kart_calib.h 的软限幅),给 400 意味着偏 1m 就出 38% 舵。原注释写的"约 1100 计数、36%"是重标中位之前的数,已订正。
     ...
     * 后者的目标速度就是"该点录制速度 × 本倍率"再夹上限(算在 kart_playback_ol_target_v() 里,它读 PARAM_PB_REVSCL;原注释写的 kart_playback.c:754 已对不上,这里改记函数名,免得那个文件一动又烂)。 */
     ...
     * 上限 1133【不等于转向软限位 —— 原注释这句是错的】:软限位是 1064(kart_calib.h 里 1248-164-20),硬行程 ±1084,1133 比两个都大,量程顶上那 69 个计数根本打不出去,纠偏顶到 1064 就被软限位截断。上限值本身赛后不动,只订正说法。
     * 【生效范围只有一处】前进复现里夹着的倒车段(钳位在 kart_playback_poll() 的倒车分支,判段靠录制速度符号;原注释写的 kart_playback.c:482 已对不上)。
     * 科目三那段独立开环倒车【不看它】,两条路径都用死宏 OL_CORR_MAX=400
     * (两处钳位分别在 kart_playback_poll_closedloop() 和 kart_playback_poll_openloop() 里,各一对;原注释写的 kart_playback.c:690/794 已对不上)—— 科三倒车拐不进去调本项没用(原注释写的"科四"是笔误,本项目只有科目一/二/三),详见 kart_params.h。
 ...
 *   【踏板驾驶也是速度消费者】v_top 取的是全体消费者的 max,里面包含
 *     KART_PEDAL_MAX_V_MS(kart_calib.h,现值 4.0f)。按 4.0 反解是
 *     1.3*(4.0+0.10)/0.00046 = 11587,已经超过 CEIL,所以 param_derive_imax()
 *     返回的是被夹住的 10000,那 1.3 倍余量实际只拿到 1.12 倍。够用 ——
 *     10000 duty 按同一条拟合支撑 4.5 m/s,仍高于 4.0;但要再往上提踏板上限,
 *     反解这条路已经到顶,得从 duty 满量程本身想办法。
 *   【放大它为什么安全】Imax 是天花板不是油门:变大不会让车超过命令速度,只是
 *     拆掉那道拿不到 31% duty 的人为限制。电流/温度由速度目标决定,那个还在
 *     人手里。代价是瞬态积分能积更多,起步/出弯可能有点速度超调,Ramp Step 兜着。
 *   【27% 和 31% 是同一道墙的两种量法】本文件上方 Spd Imax 那行(和 kart_params.h)
 *     写 27% = 10000-7300,拿日志实测峰值 duty 算;这里写 31% = 10000-6876,拿
 *     稳态 Kp*e+I 算。都对,别拿一个去改另一个。
