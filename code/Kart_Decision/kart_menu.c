 ...
 * 【现场改不了这个数】"距离更远要减小它"只能靠重新编译:本变量全文只有两处
 * 引用 —— 这行定义和 vision_roi_stat() 那次传参,没有任何按键写它。
 * menu_draw_camera 底部那句提示写着 "L2 ROI",听着像有个键能调 ROI,其实五向
 * 的键位早就分完了(RIGHT/旋钮=列、UP/DOWN=行、MID=切叠加层、LEFT=返回),
...
static void menu_draw_s3_live(void)
{
     ...
     * 立刻拨 SW3 到低挡停车。那句 delta = ... 去 kart_follow.c 里按原样搜。 */
 ...
 * 【抢 ch[39] 的有四家】除了上面这两处,还有 #if WIFI_ENABLE 那一档写
 * g_wifi_link,以及另一处写 v->valid。看到 CH39 有数【先确认编译档位再解释
 * 它】,四家的量纲完全不同。
...
static void menu_draw_s3_run(void)
{
    ...
    switch(kart_mission_get_subject3_stage())
    {
        case S3_PHASE1_FOLLOW:
            /* 从跟随段到倒车段只能手动发车:START 下降沿或油门全开沿,由
             * kart_mission.c 检,车停住/视觉丢失/MID 键都不会自动推进任务。
             * 两种控制源文案不同(S3_FOLLOW_SRC),别让现场看着遥控提示去举板子。
             * 下面 #else 那三行遥控文案和 " auto  START now"、本页
             * " hide board=back" / " hide 2s=back" 都还写着会自动返回,那是错的
             * (字串属于代码,本次不动):要改就改成 "START to go back"。 */
