...
#include "kart_flash.h"     /* FLASH_S2R_FIRST_SLOT / _SLOT_NUM:返程槽号映射 */
...
#include "kart_person_link.h"   /* 科目三 PLINK 源:TC4D7 人体跟踪帧 → kart_vtrack */
 ...
 * 阈值见 S2_RETURN_GOTO_TICKS。由 subject2_loop 的 S2_RETURN_GOTO 分支累加。 */
...
/* 科目三 START 请求锁存：允许车辆仍在减速/跟随时按下 START，不能因
 * 本拍视觉控制刷新而丢失按键事件。 */
static uint8 subject3_start_requested = 0;
...
static uint16 s3_vision_age_ticks = S3_VISION_MAX_AGE_TICKS;
...
static void mission_stop_all(void)
{
    ...
    /* 2. 后轮目标清零 + 关速度环(kart_playback_stop 已做,这里兜底保证) */
...
static void mission_enter(kart_mission_mode_t mode)
{
    switch(mode)
    {
        ...
        case MISSION_SUBJECT_2:
            ...
            /* 2026-07-29 全局坐标系:整个科目二【只在这里清一次】kart_odom。
             * 从此刻起 kart_odom (x,y) 就是"发车区坐标系"里的绝对位置,车跑完
        ...
        case MISSION_SUBJECT_3:
             ...
             *   ① kart_odom 清零(录制起点车体系对齐);② 开录制(CPU2 采样,同帧记打角+里程);
             *   ③ 控制源初始化。
             * 注意:倒车靠 kart_odom(里程或位置)映射,故录制到倒车全程 kart_odom 不再 reset。 */
            ...
            subject3_start_requested = 0;
            subject1_start_key_last = gpio_get_level(BOARD_START_KEY_PIN);
            kart_odom_reset();
            kart_odom_set_active(1);
            kart_debug_uart_set_event(EVENT_S3_ENTER, EVENT_LEVEL_INFO);
            ...
            s3_vision_age_ticks = S3_VISION_MAX_AGE_TICKS;
            s3_vision_seq = 0;
            s3_vision_frames_seen = multicore_vision_frames();
            kart_record_start();
...
static void subject1_loop(void)
{
    ...
    {
        case S1_WAIT_START:
            /* 等 START 键下降沿。触发后启动 kart_playback 复现现场手推的绕桩轨迹
             * (绕桩方向已烘在轨迹里)。kart_playback_start 内部会开速度环+航向外环。 */
        ...
        case S1_CONE_ROUTE:
             ...
             * kart_playback 到终点(A 点附近)会自动 stop → is_running 变 0,车随即停稳。 */
...
static void subject2_loop(void)
{
     ...
     * 交接是无缝的:固定动作一结束(kart_motion 不忙)下一拍遥控就自动活,
    ...
    switch(subject2_stage)
    {
        case S2_RETURN_GOTO:
        {
            ...
            if(gs == KART_MOTION_GOTO_DONE)
            {
                ...
                if(!kart_motion_is_busy())
                {
                     ...
                     * GOTO 是靠 kart_odom 把车开到"kart_odom 认为的集结点"上的,所以此刻
                     * kart_odom 位姿与录制原点【同系且已对齐】,按录制原点启动才能让
                    ...
                    subject2_return_ticks >= S2_RETURN_GOTO_TICKS)
            {
        ...
        case S2_RETURN_PLAYBACK:
             ...
             * 到终点 kart_playback 自己 stop → is_running 变 0,车随即停稳。
...
static const kart_vtrack_result_t *s3_vision_to_vtrack(const kart_vision_result_t *vis)
{
    ...
    if((vis == NULL) || (!vis->valid))
    {
        ...
        vt.frames_since_detector = (uint16)VTRACK_MAX_FRAMES_SINCE_DET;
        return &vt;
    }
    ...
    if(vis->dist_m > 0.05f)
    {
        vt.scale_r = FOLLOW_L_NOMINAL_M / vis->dist_m;
    }
    else
    {
        vt.scale_r = 1.0f;
    ...
    if(vt.scale_r >= VTRACK_SCALE_NEAR_THRESH)
    {
        vt.scale_level = KART_VTRACK_SCALE_TOO_NEAR;
    }
    else if(vt.scale_r <= VTRACK_SCALE_FAR_THRESH)
    {
 ...
 * 没有新帧的拍不重算视觉,直接把上一拍的 kart_follow 输出继续下发 —— 因为
...
static void subject3_vision_follow_tick(void)
{
     ...
     * 年龄照常增加 —— 这才是 kart_follow 眼里真实的信息新鲜度。 */
    if(multicore_vision_frames() != s3_vision_frames_seen)
    {
        s3_vision_frames_seen = multicore_vision_frames();
        vis = multicore_vision_get();
        s3_vision_age_ticks = 0;
        s3_vision_seq++;

        /* 【安全关键,不能删】告诉 kart_follow 本拍是帧沿。
         * NEAR_STOP 的防抖计数只在帧沿累加(见 kart_follow.h NEAR_STOP_FRAMES),
         * 这里不调 = 计数永不推进 = 近距保护永不锁存 = 车不会为人减速。
         * 必须在下面 kart_follow_update() 之前,而且只在真出新结果的这一支里调。 */
        follow_note_new_frame();
    }
    else
    {
        /* 本拍没有新帧:沿用上一次的视觉结果,但只在快照还新鲜时沿用。
         * 相机状态不足以兜底 —— RUNNING 由 VSYNC 判定,DMA 停了照样 RUNNING,
         * 那时 kart_vision_get() 一直交出 valid=1 的旧值,kart_follow 拿几百毫秒前的
         * 方位角打方向,表现是照着已经不在那儿的目标转。超龄就当无目标,
         * 交给 kart_follow 的丢失分支斜坡减速。 */
        if(s3_vision_age_ticks < S3_VISION_MAX_AGE_TICKS)
        {
            s3_vision_age_ticks++;
        }
    ...
    /* vis==NULL 或 vis->valid==0 时 kart_follow 内部走丢失分支(先保持,再斜坡减速)。
    ...
    /* 只在状态边沿记录；同一拍按故障/停车/识别原因的优先级只报一个。 */
    {
        ...
        {
            event = EVENT_FOLLOW_RECOVERED;
        }
        else if(fo->saturated && !subject3_follow_prev_sat)
        {
            event = EVENT_FOLLOW_STEER_SAT;
            level = EVENT_LEVEL_WARNING;
        }
        else if(!fo->saturated && subject3_follow_prev_sat)
        {
            event = EVENT_STEER_RECOVERED;
        }
        else if(valid && !subject3_vision_prev_valid)
        {
            event = EVENT_VISION_ACQUIRED;
        }
        else if(!valid && subject3_vision_prev_valid)
        {
            if(reject == VISION_REJ_AREA)       event = (vis->area_px == 0U) ? EVENT_VISION_NO_TARGET : EVENT_VISION_REJ_AREA;
            else if(reject == VISION_REJ_WIDTH) event = EVENT_VISION_REJ_WIDTH;
            else if(reject == VISION_REJ_ASPECT)event = EVENT_VISION_REJ_ASPECT;
            else if(reject == VISION_REJ_FILL)  event = EVENT_VISION_REJ_FILL;
            else                                     event = EVENT_VISION_NO_TARGET;
            level = EVENT_LEVEL_WARNING;
        }
 ...
 *       → kart_person_link_poll(10) 合成 kart_vtrack → 本函数 kart_follow_update()
 *       → 转向内环目标角 + 速度环目标速度。
 *
 * 与 VISION 源的三个不同：
...
static void subject3_loop(void)
{
     ...
     * 而遥控源那段的急停是藏在 kart_remote_control_update 里的,自动源不调它,
    ...
    switch(subject3_stage)
    {
        case S3_PHASE1_FOLLOW:
             ...
             * 车头不掉转,不搬车,不掉头,kart_odom 不 reset(倒车里程接着录制里程算)。 */
            /* START 必须优先于视觉跟随输出采样。按下后锁存请求，下面本拍
             * 不再继续下发前进控制，避免“必须等车完全停下再按”的现象。 */
            if(subject1_start_pressed())
            {
            ...
            if(subject3_start_requested)
            {
                ...
                if(n < 2)
                {
                    kart_debug_uart_set_event(EVENT_S3_ABORT, EVENT_LEVEL_ERROR);
                    mission_stop_all();
                ...
                {
                    kart_debug_uart_set_event(EVENT_S3_REVERSE_START, EVENT_LEVEL_INFO);
        ...
        case S3_SIGNAL:
             ...
             * 正常流程是裁判问完就手动 KART_LEFT 退出,这道闸只防没人管。 */
            if(subject3_signal_ticks < 0xFFFFU) { subject3_signal_ticks++; }
            if(subject3_signal_ticks >= S3_SIGNAL_TIMEOUT_TICKS)
            {
                kart_debug_uart_set_event(EVENT_S3_DONE, EVENT_LEVEL_INFO);
                kart_mission_set_mode(MISSION_REMOTE);
                return;
...
void kart_mission_init(void)
{
     ...
     * 也就不存在过去 kart_menu/kart_mission 对同一脚配两种模式、谁后 init 谁生效的问题。 */
...
static uint8 subject2_load_return_slot(uint8 slot_index, float *tx, float *ty, float *tyaw)
{
    ...
    if(slot_index >= FLASH_S2R_SLOT_NUM) return 0;

    slot = (uint8)(FLASH_S2R_FIRST_SLOT + slot_index);
 ...
 * (满舵 R = 1480/1064 = 1.39m,kart_calib.h 的 KART_STEER_MIN_RADIUS_M;
 *  原注释写的 1.32m 是重标中位之前的值,已订正 —— 圆更大,这条约束只会更紧),
 * 前置点落进最小转弯圆里就出不来。这是几何硬限制,换控制律
 * 也一样(见 MOTION_GOTO_LEAD 的注释)。失败时 GOTO 判 FAULT 停机,
 ...
 * 按录制原点启动要求 kart_odom 世界系与录制时同一系 —— 车是被人开回来的,一路的
 * 里程/航向积分误差都算在 kart_odom 上,此刻 kart_odom 认为的"集结点"未必是真集结点;
 * 而人眼看到的车【确实】在集结点上。这时该信人眼:按当前位姿启动,路径原点
 * 就钉在车现在站的地方,等于把 kart_odom 的累计误差一次性归零。
