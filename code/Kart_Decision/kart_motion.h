...
/* -------------------- 转向中位偏置(编码器计数) --------------------
 * 【下面这段标定结果(-25、汇总 -31.4)是 2026-07-30 重标中位之前的,现值 0,见本节末】
 ...
 * 【结论有效,但下面 1~3 条的算例已作废 —— 靠的是第 4 条加实车,不是这三条】
 *   作废的原因:这三条全建立在"中位偏置 = -25 计数"上。重标中位后
 *   KART_STEER_CENTER_OFS 是 0(见上一节),delta=-25 这个前提没了 ——
 *   没有 -25,就没有 25/30 ≈ 0.83° 的常驻误差,也谈不上"落在 63 计数死区里"。
 *   1. head_pid 的输出被直接写进 target_delta,【绕开 motion_set_delta】
 *      => 拿不到中位偏置补偿。通道差异是真的,但当前偏置是 0,这条现在值 0 计数。
 *      留着的意义:哪天重标出非零偏置,本地这套自动受益,head_pid 那条不会。
 *   2. 它是纯 P(KI=0):任何常驻扰动(路面横坡、轮胎跑偏、残余不对中)都得靠
 *      常驻航向误差顶住,不收敛到 0。机制成立,只是不再有 -25 这个具体算例。
 *   3. 转向内环有静摩擦死区(实测 |duty|<≈950 不动,内环 Kp=15 => 63 计数)。
 *      【这条其实对 head_pid 有利,别再拿它当反对理由】同样 63 计数,
 *      head_pid 的 30 计数/度对应 2.1°,本地 KP=25 对应 2.5° —— 本地这套的
 *      死区角反而更大。真正的差别在第 1 条的偏置通道,不在死区。
 *   4. 【决定性的一条】head_pid 是科目一/四/遥控共用的,为科目二调它会波及
 *      复刻,不能动。加上本地这套已实车跑通(用户验证倒车"挺稳的"),
 *      所以维持现状:本模块所有"要走直"的段都走 motion_yaw_corr_delta。
 *
 ...
 *   也不会瞬间怼到软限位。软限位现在是【对称的 ±1064】,由 kart_calib.h:117/118
 *   从硬限位 raw 派生;本文件原先几处写的 +1103/-1048 是旧标定的值,
 *   已全部订正,结论都不变。
 ...
 *   旧主循环退化成 5ms 拍 => 1.0s。最坏 1064 计数 / 1800 计数每秒 ≈ 0.59s,
 ...
 * 打角取接近软限位(对称 ±1064,见 kart_calib.h 第四节),留 114 计数余量:
 ...
 * 【顺时针半径比逆时针大 —— 这条现在不成立了】它整个建立在"中位偏置 = -25 计数"
 *   上:±950 不补偿会实际生效成 左 975 / 右 925 => R 1.52 / 1.60m,原注释接着写
 *   "已由 DELTA_CENTER_OFS 统一补掉"。
 *   【2026-09-06 订正】2026-07-30 重标中位之后 KART_STEER_CENTER_OFS 是 0.0f,
 *   motion_set_delta() 下发的是 delta + 0,一点没补 —— 也不需要补:新中值 164
 *   就是硬限位几何中点,左右软限本来就对称(±1064)。所以现在 ±950 就是左右各 950,
 *   左右半径本来就一样,上面那个 1.52/1.60 的算例只是历史。
 *   本节前面第四节已经写过这件事,这一段是漏改的。哪天重标出非零偏置,
 *   motion_set_delta() 会自动把它补上,这个算例也就重新有效。 */
 ...
 * 故单独给 950(与 CIRCLE_DELTA 同,离软限位 ±1064 还有 114 计数余量)。 */
 ...
 * 收敛】,没算车得先【转到轴线方向】—— 满舵 R = 1480/1064 = 1.39m
 * (kart_calib.h 的 KART_STEER_MIN_RADIUS_M;原注释写的 1.32m 对不上,
 * 数已订正,结论不变),掉个 90 度弯本身就吃掉 2.19m 弧长。
 * 2.5m 引入段大半被转向吃了,剩下的不够收敛。
 ...
 * 落进半径 1.39m 的最小转弯圆内,车只能绕着它转。换控制律也治不了(试过 Stanley
