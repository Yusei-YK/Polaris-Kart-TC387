 ...
 *
 * 【蛇形摆幅递减的成因】症状是第一摆幅度大、后面越来越小、摆不回来还走斜。
 * 【路程取模】翻转(每 HALF_DIST 翻一次)做不到:方向盘不是舵机,是有刷电机
 * 带死区、有转速上限,实测满输出转向速度约 1800 计数/秒:
 *   第一摆从中位 0 打到 +900,只要 900 计数 ≈ 0.5s ≈ 0.41m,剩下 1.19m 都在真转弯
 *   => 摆幅大;之后每次要从 -925 打到 +875,1800 计数 ≈ 1.0s ≈ 0.82m,半周期 1.6m
 *   里一半以上耗在"方向盘在路上",真正打住的路程只剩 0.78m => 摆幅一路变小。
...
/* -------------------- 动作结束回正方向盘 --------------------
 * 【动作结束必须回正】症状:左转/右转/转圈结束后轮胎还是打死状态,没有回正。
 *   影响不小,必须修:
 ...
 * 【左右半径本来就一样】KART_STEER_CENTER_OFS 是 0.0f,motion_set_delta() 下发的
 *   就是 delta 本身;新中值 164 是硬限位几何中点,左右软限对称(±1064)。哪天重标
 *   出非零偏置,motion_set_delta() 会自动把它补上。 */
 ...
 * 打角 950 对应 R≈1.56m,90° 弧长 2.45m,场地宽 >10m 完全够,不动。上面
 * CIRCLE_DELTA 注释里那组 R*|delta| ≈ 1480 就是标定结果,其中两组数据本来就是
 * 左右转 90° 实测的(-1000→1.431/1.512m、+1100→1.316m)。 */
 ...
 *
 * 【LEAD 取 6.0,依据是仿真扫参,不是上面那段指数估算】只算【横向偏差的收敛】
 * 的话 2.5m 就够,错在没算车得先【转到轴线方向】—— 满舵 R = 1480/1064 = 1.39m
 * (kart_calib.h 的 KART_STEER_MIN_RADIUS_M),掉个 90 度弯本身就吃掉 2.19m 弧长。
