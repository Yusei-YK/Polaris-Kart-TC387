 ...
 *       不写行号:改踏板那批往 kart_menu.c 里插了二十多行、往 kart_mission.c
 *       加了几行,把行号引用整体顶了下去(kart_record.h 那六处也一样)。
 *       函数名不会这样漂。调试口现在一处都不启动复现。
 *   kart_playback_poll()   —— 【5ms 一拍】cpu0_main.c 的 kart_task_5ms() 里,
 *                             在 kart_steer_abs_update 之后、
 *                             kart_steer_ctrl_update 之前(好覆写目标航向)。
 *                             本文件所有拍数(刹车 HOLD/MAX 等)都按 5ms/拍折算,
 *                             换调用点得连它们一起改。
 ...
 * 下界依据:满舵需 0.59s(1064 计数 ÷ 1800 计数/s;原注释写的 1103 计数是重标
 *   中位之前的行程值,软限幅实为 ±1064,数已订正,结论不变),60 脉冲=4.4 m/s 时
 ...
 * ±0.12/m 曲率噪声,而满舵曲率只有 0.72/m(=1/1.391,满舵半径见 kart_calib.h
 * 的 KART_STEER_MIN_RADIUS_M;原注释写的 0.76 是 1/1.32,那个半径是重标中位
 * 之前的值,已订正)—— 噪声占满量程 16%,
 ...
 * 满锁 1064 计数(kart_calib.h 的软限幅,本文件后面也写了这个数),给到 800
 * 就是允许纠偏打到 75% 舵。原注释写的"约 1100 计数、七成舵"对不上,已订正。 */
 ...
 *   别按行号数下去 —— 附近还有 S3 OLMode 和 S3 OL Kh 两个长得很像的槽,
 *   数错了会调到别的旋钮上)。
 ...
 *      (在 kart_debug_uart.c 的 #if LOG_PROFILE_S3 那段里;另注意 51 通道
 *      全量档里 hue_pct 是 CH20,两档别混),
