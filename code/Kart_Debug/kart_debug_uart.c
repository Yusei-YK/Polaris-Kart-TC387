 ...
 * - 主循环每 4 tick(50Hz)发送一帧，长度随编译的通道档变:
 *   【出厂档】LOG_PROFILE_S3=1 → 43 个小端 float32 + 帧尾 00 00 80 7F,共 176 字节;
 *   【全量档】LOG_PROFILE_S3=0 → 51 路,共 208 字节。
 *   原文只写了全量档那一个数,当前编译的并不是它；
 ...
 * 2026-08-14 由 48 加到 50:新增 CH48 跟随目标速度、CH49 跟随目标转角。
 * 后来又补了 CH50 视觉置信度,所以全量档现在是 51 路(ch[0..50])。
 * 【别按这个演变史去设 VOFA】上面每一步都只是历史。以 KART_LOG_CHANNELS 为准:
 * 出厂档 43 路,全量档 51 路。数对不上就帧长不匹配、波形全体错位。 */
/* 【日志通道组】1 = 科目三跟随剖面(现为 43 路),0 = 全量 51 路(科目一/二/四用)。
 * 【剖面不等于很短】最初它只有 26 路,后来逐步补回逆光诊断、搬核耗时、
 * 手推轨迹和倒车横向环,现在是 43 路 —— 相对全量档只省了 8 路。
 * 精简的理由是分析耗时:列可以多,但得按看的顺序排,跟随段真正在变的那几路
 * 得挤到前面,不能散在 kart_playback/遥控/kart_odom 中间。
 * 【改这个宏必须同步改 VOFA 的通道数】否则帧长不匹配、波形全体错位。 */
#define LOG_PROFILE_S3         (1)

#if LOG_PROFILE_S3
#define KART_LOG_CHANNELS           (43U)  /* VOFA 通道数。这个数不是工程的固定属性,
                                            * 由上面的 LOG_PROFILE_S3 选:=1 走本档(当前
                                            * 调科目三用的剖面),=0 走下面那档。帧长 =
                                            * 通道数*4+4,本档 43*4+4=176B。
                                            * 【烧录前把 VOFA 主机端的通道数设成实际编译的
                                            * 那一档】否则帧长不匹配、波形全体错位。
 ...
 * 一帧 43ch=176 字节 @460800 直发≈4.6ms(全量档 51ch=208B 约 5.4ms),
 * 放 5ms 调度里会把控制拍打爆。
...
#define KART_LOG_RING_SIZE          (512U)  /* 2 的幂,位与回绕。留 1 字节区分满/空,
                                             * 可用 511B;按出厂档 176B/帧只装得下 2 帧。
                                             * 装不下是丢整帧(计入 CH37),不是排队等。 */
...
void kart_debug_uart_poll(void)
{
...

    {
...
#if LOG_PROFILE_S3
        {
             ...
             * 【不读相机曝光值】那要借 UART1 9600,运行时调它既抢外设又阻塞。
             * 注意不是"init 后已还给灯板" —— kart_camera.c:226 写明根本
             * 不需要还:UART1 静态归灯板,由 dot_matrix_screen_init() 配成 2Mbps。 */
            ...
            /* CH37 环形缓冲丢帧计数。出厂档帧长 176B,一帧要 11 次
             * background_poll(每次排 16B),这一路是"日志本身可不可信"的唯一判据:
             ...
             * 判读只看 CH1(阶段)==1 的那一段。
             *
             * 判读方法(对应 kart_playback_poll_closedloop 末尾写 cur/aim 那段注释;
             *  原文引的那段行号现在是 kart_playback_ol_target_v,已挤走):
         ...
         * 这6路的分支切换不改变通道总数。
         * 【但 CH39-CH44 只在全量档存在】出厂档 LOG_PROFILE_S3=1 只到 ch[42],
         * 下面 WiFi / 人体链路 / 本地视觉三套赋值都在 #else 里,根本编不进去。
         ...
         *             3=在线
           CH40 frame_ok 累计好帧。日志帧是 50Hz 发的(与本档是 43 路还是 51 路无关)，若 4D7 推理 20Hz，
         ...
         * 下面,WIFI_ENABLE 改回1时把两段对调即可,通道总数不变(本段所在的全量档为 51 路)。
         *
