...
/*
 * 硬件自测(临时) —— IPS200 屏(软件 SPI) + 旋钮编码器 + 5 向按键
 * ------------------------------------------------------------------
 * 目的:验证 v2 主板的屏能否点亮/不花屏,旋钮和按键能否读到。
 *      【屏不是并口的】本文件走的是 ips200_init(IPS200_TYPE_SPI),即软件 SPI,
 *      屏幕丝印是 BLK/CS/DC/RST/SDA/SCL(见下面 .c 里那段说明,以及
 *      board_pins.h 的 IPS200 段落,那里也写明"当前为软件 SPI")。
 *      早期版本那套并口接法的说法和引脚表都是换板前留下的,别照着接线。
 ...
 *        然后常驻显示旋钮计数 + 7 路输入的实时电平
 *        (5 向 UP/DOWN/KART_LEFT/RGHT/MID,加旋钮按下 ESW,加发车键 STRT)。
 ...
 *      【但那三个脚号现在都还活着,只是换了身份】排查时别按旧表对号:
 *        P20.6 = 旋钮按下(BOARD_ENC_SW_PIN,屏上 ESW 那行)
 *        P20.7 = 发车键 SW3(BOARD_START_KEY_PIN,屏上 STRT 那行)
 *        P33.4 = 五向的 MID(BOARD_KEY_MID_PIN)
 *   【屏的引脚不在这里查】原来这行列的 D0~D7 = P11.9~P11.12 / P13.0~P13.3
 *   和控制线 P15.0~P15.5 是并口屏的接法,本文件不用。其中 P13.0/P13.1
 *   今天是 UART_10,也就是跑日志的那个口 —— 照旧表接线会直接撞掉日志。
 *   现在的屏接法一律看 board_pins.h 的 IPS200 段落。
 */
