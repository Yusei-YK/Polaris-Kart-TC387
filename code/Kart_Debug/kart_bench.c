 ...
 *
 * 【2026-08-26 的现状】还是这个状态，一项都没填。整个赛季的实时性问题是靠 VOFA
 *   通道解决的（分发耗时 CH26、视觉单帧 CH29），这套基准从没上过场。
 *   kart_bench_init() 开机被调一次，但只把统计数组清零；run_all / run_one 没有
 *   任何调用点。为什么留着见 kart_bench.h 头部的【当前状态】。
 ********************************************************************************************************************/
 ...
 * 运行单项基准
 *
 * 【TODO】B1-B7、B10 的具体实现待硬件验证时填充,现在这些 case 是空的。
 *
 * 【但统计量照样在更新】空 case 也会 break 出来走到下面的 t1/dt_us,
 * 然后调 update_stat 并把 dt_us 返回 —— 返回的不是 0(只有 default:
 * 那条非法 item 的路径才直接 return 0)。所以空项会记下一堆接近 0 的
 * "耗时",看起来像量过了。谁要读统计量,先确认对应 case 里真有东西。
 *
 * 【B11/B12 有副作用】那两项调的是真的速度环和转向环,会下发占空比,
 * 详见 kart_bench.h 里 run_one 的说明。
 *-----------------------------------------------------------------------------------------------------------------*/
